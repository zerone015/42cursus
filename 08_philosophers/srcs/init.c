...
/*   Updated: 2022/09/09 23:02:53 by yoson            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_philo(t_info *info)
{
	...
	while (++i < info->num_of_phil)
	{
		...
		info->philo[i].right_fork = (i + 1) % info->num_of_phil;
...
void	init_info(t_info *info, char *argv[])
{
	...
	info->fork = safe_malloc(sizeof(pthread_mutex_t) * info->num_of_phil);
	init_mutex(info);
	info->philo = safe_malloc(sizeof(t_philo) * info->num_of_phil);
	init_philo(info);
}
